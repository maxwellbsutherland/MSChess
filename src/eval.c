...
void initialize_evaluation(void) {
    for (U8 p = WP; p <= BK; p++) {
        for (U8 sq = 0; sq < 64; sq++) {
            const int t = TYPE(p);
            const U8 psq = (COLOR(p) == WHITE) ? flip(sq) : sq;

            mg_table[p][sq] = mg_value[t] + mg_pesto_table[t][psq];
            eg_table[p][sq] = eg_value[t] + eg_pesto_table[t][psq];
        }
    }
}
...
int evaluate(const Board *board) {
    ...
    for (int p = WP; p <= BK; p++) {
        ...
        while (bb) {
            ...
            mg[color] += mg_table[p][sq];
            eg[color] += eg_table[p][sq];
